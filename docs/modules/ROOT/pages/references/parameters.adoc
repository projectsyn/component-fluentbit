...
Select whether fluent-bit is deployed as a `DaemonSet` or `Deployment`.
...
Number of fluent-bit replicas when `kind` is `Deployment`. Deployments with more than one replica receive a PodDisruptionBudget.
...
Maximum number of fluent-bit pods that may be unavailable during a disruption when a Deployment has more than one replica.
