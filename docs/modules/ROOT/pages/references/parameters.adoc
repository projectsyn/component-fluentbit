...
For `Deployment` deployments with `replicaCount` greater than 1, a
PodDisruptionBudget is enabled automatically. Pod anti-affinity is always
added. The `podDisruptionBudget.maxUnavailable` value can be overridden via
`helm_values`.
